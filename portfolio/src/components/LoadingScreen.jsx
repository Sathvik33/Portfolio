    ...
    >
      <div className="w-full max-w-sm px-8">
        <motion.div className="mb-10 flex items-center gap-3" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          ...
          <span className="font-mono text-sm text-[var(--t3)] tracking-widest uppercase">MSR.portfolio</span>
        </motion.div>

        <div className="mb-6 space-y-1">
          {lines.map((line, i) => (
            <motion.div key={i} className="font-mono text-xs" initial={{ opacity: 0, x: -10 }} animate={{ opacity: phase >= i ? 1 : 0.1, x: 0 }} transition={{ delay: i * 0.15 + 0.2 }}>
              <span className={phase >= i ? (i === 3 ? 'text-[#00d4ff]' : 'text-[var(--t3)]') : 'text-[var(--border)]'}>
                {line}
              </span>
            </motion.div>
          ))}
        ...
        <div className="relative h-[2px] w-full overflow-hidden rounded-full bg-[var(--border)]">
          <motion.div className="h-full rounded-full bg-[#00d4ff]" style={{ width: `${progress}%` }} transition={{ duration: 0.1 }} />
          <div className="absolute top-0 h-full w-8 blur-sm bg-[#00d4ff]/60" style={{ left: `${progress - 4}%` }} />
        </div>
